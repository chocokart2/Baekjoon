...
namespace _2623try1
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string[] peopleAndLine = Console.ReadLine().Split(' ');
            NodePtr[] peoples = new NodePtr[int.Parse(peopleAndLine[0])];
            ...
            for (int t = int.Parse(peopleAndLine[1]); t > 0; --t)
            {
                //Console.WriteLine($">> {t}");
                string[] numArrayStr = Console.ReadLine().Split(' ');
                int[] numArray = new int[int.Parse(numArrayStr[0])];
                for (int index = 1; index < numArrayStr.Length; ++index)
                {
                    numArray[index - 1] = int.Parse(numArrayStr[index]);
                }
                for (int index = 1; index < numArray.Length; ++index)
                {
                    int leftIndex = numArray[index - 1] - 1;
                    int rightIndex = numArray[index] - 1;
            ...
            if (visitedLeft == 0)
            {
                ...
                {
                    answer.AppendLine(result[index].ToString());
                }
