...
namespace no1225try1
{
    internal class Program
    {
        ...
        {
            string[] numsNK = Console.ReadLine().Split(' ');
            int count = int.Parse(numsNK[0]);
            int target = int.Parse(numsNK[1]);
            Heap<int> selecter = new Heap<int>(count, delegate(int left, int right) { return left - right; });
            for (int i = 0; i < count; i++)
            {
                string[] nums = Console.ReadLine().Split(' ');

                selecter.Push(int.Parse(nums[1]) - int.Parse(nums[0]));
            }
